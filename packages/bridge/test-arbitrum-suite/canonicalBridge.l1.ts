...
describe('Bridge peripherals layer 1', () => {
  ...
  it('should escrow depositted tokens', async function () {
    ...
    let data = ethers.utils.defaultAbiCoder.encode(['uint256', 'bytes'], [maxSubmissionCost, '0x'])

    // router usually does this encoding part
    data = ethers.utils.defaultAbiCoder.encode(['address', 'bytes'], [accounts[0].address, data])

    await testBridge.outboundTransfer(token.address, accounts[0].address, tokenAmount, maxGas, gasPrice, data)
  ...
  it('should withdraw erc20 tokens from L2', async function () {
    ...
    const exitNum = 0
    const withdrawData = ethers.utils.defaultAbiCoder.encode(['uint256', 'bytes'], [exitNum, '0x'])
  ...
  it('should process fast withdrawal correctly', async function () {
    ...
    const PassiveFastExitManager = await ethers.getContractFactory('L1PassiveFastExitManager')
    ...
    const tradeData = ethers.utils.defaultAbiCoder.encode(
      ...
      [accounts[0].address, maxFee, fastExitMock.address, tokenAmount, token.address, liquidityProof, '0x'],
    )
    ...
    assert.equal(prevUserBalance.toNumber() + tokenAmount - maxFee, postUserBalance.toNumber(), 'Tokens not escrowed')
    ...
    const inboundData = ethers.utils.defaultAbiCoder.encode(['uint256', 'bytes'], [exitNum, '0x'])
    ...
    const expectedTopic = '0x2f0c0af451e6330658fba0c08f7d82acdb1feff8d2904044a765af1b27df3e1f'
  ...
  it('should submit the correct submission cost to the inbox', async function () {
    ...
    // TicketData(uint256)
    const expectedTopic = '0x7efacbad201ebbc50ec0ce4b474c54b735a31b1bac996acff50df7de0314e8f9'
    const logs = receipt.events
      .filter((curr: any) => curr.topics[0] === expectedTopic)
      .map((curr: any) => inbox.interface.parseLog(curr))

    assert.equal(logs[0].args.maxSubmissionCost.toNumber(), maxSubmissionCost, 'Invalid submission cost')
