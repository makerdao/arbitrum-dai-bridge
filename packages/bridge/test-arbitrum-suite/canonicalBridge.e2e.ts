...
describe('Bridge peripherals end-to-end', () => {
  ...
  before(async function () {
    ...
    const L1TestBridge: ContractFactory = await ethers.getContractFactory('L1GatewayTester')
    l1TestBridge = await L1TestBridge.deploy()

    const L1InboxMock: ContractFactory = await ethers.getContractFactory('InboxMock')
    ...
    const L2TestBridge: ContractFactory = await ethers.getContractFactory('L2GatewayTester')
    ...
    await l2TestBridge.initialize(l1TestBridge.address, l2RouterTestBridge.address, beaconProxyFactory.address)
  ...
  it('should deposit tokens', async function () {
    ...
    await l1RouterTestBridge.outboundTransfer(token.address, accounts[0].address, tokenAmount, maxGas, gasPrice, data)
    ...
    const l2TokenAddress = await l2RouterTestBridge.calculateL2TokenAddress(token.address)
    const l2TokenAddressFromL1Router = await l1RouterTestBridge.calculateL2TokenAddress(token.address)
    assert.equal(l2TokenAddressFromL1Router, l2TokenAddress, 'Wrong address oracle')

    const l2Token = await Token.attach(l2TokenAddress)
    const l2Balance = await l2Token.balanceOf(accounts[0].address)
    assert.equal(l2Balance, tokenAmount as any, 'Tokens not minted')
  })
  ...
  it('should withdraw erc20 tokens from L2 using router', async function () {
    ...
    await l2RouterTestBridge.functions['outboundTransfer(address,address,uint256,bytes)'](
      token.address,
      accounts[0].address,
      tokenAmount,
      '0x',
  ...
  it('should force withdraw correctly if deposit is incorrect', async function () {
    ...
    const prevUserBalance = await token.balanceOf(accounts[0].address)
    const prevAllowance = await token.allowance(accounts[0].address, l1TestBridge.address)
    ...
    const postUserBalance = await token.balanceOf(accounts[0].address)
    const postAllowance = await token.allowance(accounts[0].address, l1TestBridge.address)

    assert.equal(prevUserBalance.toNumber(), postUserBalance.toNumber(), 'Tokens not escrowed')
